...
it.effect("a rename changes the row's handle in place and nothing else", () =>
  Effect.gen(function* () {
    ...
    // row to the end; a filter would drop it; a projection that rewrote the ref
    // from the payload would pass a ref-only assertion. The other two rows are
    // asserted too, because a `map` that renamed every row matches the renamed
    // one alone.
