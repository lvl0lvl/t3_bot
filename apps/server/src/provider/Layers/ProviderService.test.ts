...
describe("agent browser access", () => {
  ...
  // always gets a credential (the pull request and comms toolkits are never
  // withheld), and `preview` on it is what actually grants or denies the
  // browser tools.
  it.effect("issues a credential without preview when agent browser access is off", () =>
    Effect.gen(function* () {
      ...
      assert.deepEqual(issued, [{ threadId, capabilities: ["comms", "pull-requests"] }]);
    }).pipe(Effect.provide(NodeServices.layer)),
  );
  ...
  it.effect("issues a credential with preview when agent browser access is on", () =>
    Effect.gen(function* () {
      ...
      assert.deepEqual(issued, [
        { threadId, capabilities: ["comms", "device", "preview", "pull-requests"] },
      ]);
    }).pipe(Effect.provide(NodeServices.layer)),
  );
  ...
  it.effect("drops only the preview capability when browser access alone is off", () =>
    Effect.gen(function* () {
      ...
      assert.deepEqual(issued, [{ threadId, capabilities: ["comms", "device", "pull-requests"] }]);
    }).pipe(Effect.provide(NodeServices.layer)),
  );
  ...
  it.effect("requests an MCP credential when the project overrides browser access to on", () =>
    Effect.gen(function* () {
      ...
      assert.deepEqual(issued, [{ threadId, capabilities: ["comms", "preview", "pull-requests"] }]);
    }).pipe(Effect.provide(NodeServices.layer)),
  );
});
